...
1. De key mag niet het woord "register" bevatten. Gebruik een naam die aangeeft wat de identifier is, niet hoe het register heet. Dus `dpl.object.algorithm_id`, *maar niet* `dpl.object.algorithm_register_id`
1. Gebruik geen keys die al in de `dpl.core` namespace bevatten. Dus geen `dpl.extensie.processing_activity_id`, omdat `dpl.core.processing_activity_id` al bestaat
1. Een verwijzing naar een register kan zowel 1 item als meerdere items bevatten. Het type van het attribuut kan dus een `String` of een `List` zijn. Voorbeeld van een `List` zou meerdere verwijzigen zijn naar het wetten register, als meerdere wetten van toepassing zijn op datgene wat wordt gelogd

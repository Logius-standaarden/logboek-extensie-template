...
Hierdoor kan het systeem ten onrechte concluderen dat de student niet meer is ingeschreven, terwijl dit niet klopt.

Maar op het moment dat er wel gebruik gemaakt wordt van unieke namespaces zoals ```student_id``` en ```exam_id``` en duidelijk onderscheid maakt, kan het systeem de velden wel duidelijk herkennen. 

Hiermee kan je gebruik maken van twee extensies tegelijkertijd en krijg je het volgende response terug:

```
  student_id: student115839
  student_status: ingeschreven
  exam_id: examenNederlands
  exam_status: gemachtigd
```
